 ...
 */
 ...
 public:
 	...
 	~Arreglo( );
 	void agregar(T elem);
 	template <typename G> friend std::ostream& operator<< (std::ostream& out, const Arreglo<G>& arreglo);
 	Arreglo operator+(const Arreglo& arr2);
 	Arreglo& operator=(const Arreglo& arreglo); //MAL
 	...
 	template <typename T> Arreglo<T> Arreglo<T>:: operator+(const Arreglo<T>& arr2) {
 	   ...
 	   Arreglo<T> arreglo;
 	    for (int i = 0; i<_nElem; i++){
 			arreglo.agregar(_elems[i]+arr2._elems[i]);
 		}
 		return arreglo;
 	};
