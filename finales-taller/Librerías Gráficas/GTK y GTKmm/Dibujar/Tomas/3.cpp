	...
	private:
		bool on_expose_event(GdkEventExpose* ev) {
			...
			if (window) {
				...
				std::vector< double > vLinea(2); //Creo un vector de una posicion
				vLinea[0] = 0.04; //En la primer posicion del vector se indica el espaciado
				vLinea[1] = 0.01; //En la primer posicion del vector se indica el espaciado
				ctx->set_dash(vLinea, 0);
